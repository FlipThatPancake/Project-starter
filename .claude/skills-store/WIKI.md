...
Purpose: feed a downstream model deciding overlaps/dependencies for the skill-curator install flow. Every filename below is quoted from the actual source, not inferred.
...
| **duplicate skill across packs** | `gsap-scrolltrigger` exists verbatim by NAME in BOTH github:greensock/gsap-skills AND github:freshtechbro/claudedesignskills (different content/maintainer) | install from ONE source only; provenance is tracked in `LOCK.md`'s `source` column to prevent double-install under the same skill name |
