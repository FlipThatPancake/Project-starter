...
2. DIFF the things OUR SYSTEM depends on and report each before applying:
   ...
   - modules (`reference/` etc.) → add/remove `module-index.md` rows to match.
   - new conflicts → a new module/dep may collide with an installed skill; add a `conflict-rulings.md` rule if so.
...
2. **Schema changed → look into the project**: glob §2a for the file.
   ...
   - Present → run `scripts/structcheck.sh <project-file> "<new marker>" ...` with the NEW signature's markers. It reports which required markers the existing file is MISSING (renamed/removed/new sections). Also re-run the footprint-overlap + `conflict-rulings.md` check against the skill's NEW footprint — an update may start writing a file another installed skill owns.
