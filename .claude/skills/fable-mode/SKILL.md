...
description: >
  ...
  policy: manual only — invoke by name (/fable-mode), do not self-trigger on
  "thoroughly"/"systematic" phrasing (see note below).
...
<!-- adapted from github:mrtooher/fable-mode; added disable-model-invocation: true per this repo's ruling (manual-only, not a good ride-along — invoke explicitly, don't self-trigger on phrasing). "execution-guardrails" companion skill referenced in the original is not installed here; its 3 rules are restated inline above, same as the upstream fallback text already provides. -->
