...
# Model v2: a skill is "registered" iff its own SKILL.md carries policy+category
# frontmatter (no central CATALOG). Missing either = drift. Skips the active copy
# of a skill whose store master already covers the name (shadowing).
if [ -d .claude/skills ]; then
  DRIFT=""
  for d in .claude/skills/*/ .claude/skills-store/skill-storage/*/; do
    [ -d "$d" ] || continue
    f="$d/SKILL.md"
    if [ ! -f "$f" ] || ! grep -q '^policy:' "$f" || ! grep -q '^category:' "$f"; then
  ...
  if [ -n "$DRIFT" ]; then
    ...
    if [ $((snow - slast)) -ge 3600 ]; then
      date +%s > "$SSTAMP"
      printf '{"systemMessage":"\\ud83e\\udde9 Skill loadout drift: %s missing policy/category frontmatter. Run the skill-manager skill."}\n' "$DRIFT"
      exit 0
    fi
  fi
fi
